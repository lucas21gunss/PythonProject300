        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                if dados_retorno:
                    ...
                    if 'celulas' in dados_retorno and isinstance(dados_retorno['celulas'], list):
                        msgs = [c.get('mensagem') for c in dados_retorno['celulas'] if c.get('mensagem')]
                        if msgs:
                    ...
                    return {'success': False, 'error': f'Serviço não encontrado (URL Errada).'}

            else:
                msg = response.text
                if dados_retorno and 'mensagem' in dados_retorno:
                    msg = dados_retorno['mensagem']
                return {'success': False, 'error': f'Erro Protheus ({response.status_code}): {msg}'}
