...
def init_routes(app):
    ...
    @app.route("/retiradas")
    def view_retiradas():
        """Página simples de retiradas (Visualização)"""
        # Verificação de login opcional (pode descomentar se quiser proteger)
        # if not request.cookies.get('token'):
        #     return redirect('/login')

        return render_template("retiradas.html")
    ...
    def api_celulas(projeto, revisao):
        ...
        logger.info(
            f"Usuário {g.user} solicitou células do projeto {projeto} rev {revisao}"
        )
    ...
    def api_produtos(projeto, revisao, celula):
        ...
        logger.info(
            f"Usuário {g.user} solicitou produtos - Projeto: {projeto}, Célula: {celula}"
        )
    ...
    def api_requisicao():
        ...
        try:
            ...
            if "projeto" not in dados:
                ...
                return (
                    jsonify(
                        {"success": False, "error": 'Campo "projeto" é obrigatório'}
                    ),
                    400,
                )
            ...
            return (
                jsonify({"success": False, "error": f"Dados inválidos: {str(e)}"}),
                400,
            )
    ...
    def health_check():
        ...
        return jsonify(
            {"status": "ok", "service": "Portal Manufatura MRB", "version": "2.0"}
        )
